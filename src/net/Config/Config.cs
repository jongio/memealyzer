...
namespace Memealyzer
{
    public static class Config
    {
        ...
        private static Dictionary<int, Uri> proxyTunnels = new Dictionary<int, Uri>();
        ...
        static Config()
        {
            ...
            if (UseAzurite || UseAzuriteBlob)
            {
                ...
                if (blobTunnel != null)
                {
                    proxyTunnels.Add(10000, blobTunnel);
                }
            }
            ...
            if (UseAzurite || UseAzuriteQueue)
            {
                ...
                if (queueTunnel != null)
                {
                    proxyTunnels.Add(10001, queueTunnel);
                }
            }
        }
        ...
        public static Uri GetStorageEndpoint(string type, int port, bool emulator, bool proxy)
        {
            ...
            if (emulator)
            {
                if (proxy && proxyTunnels.ContainsKey(port))
                {
                    endpoint = proxyTunnels[port].ToString() + AzuriteAccountName;
                }
                else
                {
